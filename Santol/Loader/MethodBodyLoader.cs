            ...
            foreach (Instruction instruction in instructions)
            {
                switch (instruction.OpCode.Code)
                {
                    ...
                    case Code.Conv_I:
                        ...
                        break;
                    case Code.Conv_I1:
                        PushNode(new Nodes.Convert(PrimitiveType.SByte, PopNode()));
                        break;
                    case Code.Conv_I2:
                        PushNode(new Nodes.Convert(PrimitiveType.Int16, PopNode()));
                        break;
                    case Code.Conv_I4:
                    ...
                    case Code.Conv_U2:
                        ...
                        break;
                    case Code.Conv_U4:
                        PushNode(new Nodes.Convert(PrimitiveType.UInt32, PopNode()));
                        break;
                    case Code.Conv_U8:
                        PushNode(new Nodes.Convert(PrimitiveType.UInt64, PopNode()));
                        break;
                    ...
                    case Code.Ble:
                    {
                        ...
                        NodeReference cond =
                            AddNode(new Comparison(Comparison.OperationType.LessThanOrEqual, v1, v2));
                        Block target = GetBlock((Instruction) instruction.Operand);
                        Block elseTarget = GetBlock((Instruction) instruction.Next.Operand);
                        Tuple<IType[], NodeReference[]> stack = CollapseStack();
                        target.AddCaller(block, stack.Item1);
                    ...
                    case Code.Bgt:
                    {
                        ...
                        NodeReference v1 = PopNode();
                        NodeReference cond =
                            AddNode(new Comparison(Comparison.OperationType.GreaterThan, v1, v2));
                        Block target = GetBlock((Instruction) instruction.Operand);
                        Block elseTarget = GetBlock((Instruction) instruction.Next.Operand);
                        Tuple<IType[], NodeReference[]> stack = CollapseStack();
                        target.AddCaller(block, stack.Item1);
